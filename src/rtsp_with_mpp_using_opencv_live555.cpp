...
MPP_RET init_mpp(){
    ...
    // mpp_enc_cfg_set_s32(cfg, "rc:gop", 3);
...
u_char header[8] = {0,0,0,0,0,0,0,0};
int send_packet(Command &obj){
    ...
    // int64_t timestep = av_gettime(); // 8byte
    // memcpy(packet->data,header,8);
    // memcpy(packet->data + 8,&timestep,8);
    // memcpy(packet->data + 16,header,8);
    // if(!(packet->flags & AV_PKT_FLAG_KEY)){
    //     memcpy(packet->data + 24,cExtradata,extra_data_size);
    //     memcpy(packet->data + 24 + extra_data_size,frame_data,packet_data_size);
    //     packet->size = packet_data_size + extra_data_size + 24;
    // }else{
    //     memcpy(packet->data + 24,frame_data,packet_data_size);
    //     packet->size = packet_data_size + 24;
    // }
    if(!(packet->flags & AV_PKT_FLAG_KEY)){
        ...
        */
        ...
        // /**中间追加**/
        // memcpy(packet->data,cExtradata,extra_data_size);
        // memcpy(packet->data + extra_data_size,header,8);
        // memcpy(packet->data + extra_data_size + 8,&timestep,8);
        // memcpy(packet->data + extra_data_size + 16,header,8);
        // memcpy(packet->data+extra_data_size + 24,frame_data,packet_data_size);
        // packet->size = packet_data_size + extra_data_size + 24;
    }
...
int opencv_capture(Command &obj){
    ...
    videoCap.set(CAP_PROP_FRAME_WIDTH, 1920);//宽度
    videoCap.set(CAP_PROP_FRAME_HEIGHT, 1080);//高度
    // videoCap.set(CAP_PROP_FRAME_WIDTH, 1080);//宽度
    // videoCap.set(CAP_PROP_FRAME_HEIGHT, 720);//高度
    // videoCap.set(CAP_PROP_FPS, 30);//帧率 帧/秒
    videoCap.set(CAP_PROP_FOURCC,VideoWriter::fourcc('M','J','P','G')); // 捕获格式
    // videoCap.set(CAP_PROP_FOURCC,VideoWriter::fourcc('I','4','2','0')); // 捕获格式
    // videoCap.set(CAP_PROP_CONVERT_RGB,0);
	...
	int is_init_encoder = 0;
    Mat cvframe,yuvframe,cvframeTemp;
	while (videoCap.read(cvframe))
	{
        auto now = av_gettime();
		// imshow("video_show", cvframe);
        ...
        // usleep(1/50.0f * 1e6);
        // resize(cvframe,cvframeTemp,Size(600, 480),0,0,INTER_AREA);
        // cvtColor(cvframeTemp,yuvframe,COLOR_RGB2YUV_YV12);
        cvtColor(cvframe,yuvframe,COLOR_RGB2YUV_YV12);
		...
		if(!is_init_encoder){
            ...
            /*YUV图像需要对照原始图像大小进行还原，因此需要保存此处的值*/
            // 原始图像大小
            Size size = cvframe.size();
            // Size size = cvframeTemp.size();
            width = size.width;
            height = size.height;
            // 对齐后的大小
            hor_stride = MPP_ALIGN(width, 16); // 1920
            ...
            cout << "init data and mpp finished!" << endl;
			is_init_encoder = 1;
		}
		transfer_frame(yuvframe,obj);
        if(framecount % 100 == 0){
        ...
        yuvframe.release();//释放yuvframe 否则会内存泄漏
