...
码流的固定起始码是：`{00,00,00,01}`，这个表示上一帧的结束以及本帧的数据开始。而对于`PPS`信息的开头码为：`{00 00 00 01 67}`，同样的`SPS`信息的开头码为：`{00 00 00 01 68}`。`P`帧数据的开头则是`{00 00 00 01 41}`。关键帧`I`帧的数据格式则是包含了两块自定义信息`{00 00 00 01 06}`，以及一块`{00 00 00 01 65}`的数据。
